...
class HexagonPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double radcomponents = size.height / 2;
    ...
    Path path = Path();
    path.moveTo(centerX + radcomponents * cos(0.0), centerY + radcomponents * sin(0.0));

    for (int i = 1; i <= 6; i++) {
      double x = centerX + radcomponents * cos(angle * i);
      double y = centerY + radcomponents * sin(angle * i);
      path.lineTo(x, y);
    }
